...
class MegatronEngineWithLMHead(MegatronEngine):
    ...
    def postprocess_micro_batch_func(
        ...
        if loss_function is not None:
            ...
            train_draft_only = data.extra_info.get("train_draft_only", False)

            if train_draft_only:
                # === 串行模式：Draft 训练步 ===
                # Draft loss 已经在 eagle3_patch.py 中计算并暂存
                # 这里从 model 中提取并返回
                from verl11.models.mcore.eagle3_patch import drain_draft_losses
                ...
                if draft_losses:
                    # 汇总所有 microbatch 的 draft loss
                    import torch
                    total_draft_loss = torch.stack(draft_losses).mean()
                    scaled_loss = total_draft_loss * data["num_micro_batch"]

                    metrics = {
                        "draft/loss": total_draft_loss.item(),
                        "draft/num_microbatches": len(draft_losses),
                    }
                else:
                    # 没有 draft loss（可能 draft 被跳过）
                    scaled_loss = torch.tensor(0.0, device=device)
                    metrics = {"draft/loss": 0.0, "draft/skipped": 1.0}
            else:
                # === 并行模式 或 串行模式的 Actor 训练步 ===
                # 使用正常的 PPO loss
