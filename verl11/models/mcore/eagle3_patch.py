...
def _megatron_gptmodel_postprocess_eagle3(
    ...
    """
    # 检查串行训练标志
    train_draft_only = getattr(self, "_train_draft_only", False)
    enable_draft = getattr(self, "_enable_draft_training", True)  # 默认 True 保持向后兼容
...
def _eagle3_parallel_training(
    ...
    # ---- draft path (training only, when we have labels/loss_mask) ----
...
def _eagle3_draft_training_step(
    ...
    """串行模式的 Draft 训练步（新增方法）

    【新增逻辑】专门用于串行模式的 draft 训练步，与原有逻辑完全独立。

    执行流程：
    1. 捕获 actor hidden states（actor 已经 forward，参数冻结）
    2. Draft forward
    3. 计算 draft loss（teacher = actor logits, detached）
    4. 暂存 loss 供后续 backward
    """
    output_weight = None
    if self.share_embeddings_and_output_weights:
        output_weight = self.shared_embedding_or_output_weight()
    ...
    # ---- 计算 policy logits (作为 teacher，需要 detach) ----
    logits, _ = self.output_layer(hidden_states, weight=output_weight, runtime_gather_output=runtime_gather_output)
    logits = logits.transpose(0, 1).contiguous()  # [s b v] -> [b s v]
    ...
    if draft is None or capture is None:
        logger.warning("[DRAFT-TRAIN-SERIAL] Draft model or capture not found, skipping")
        return logits

    if loss_mask is None:
        logger.debug("[DRAFT-TRAIN-SERIAL] No loss_mask, skipping draft training")
        return logits

    try:
        # 1. 获取 captured hidden states
        ...
        # 2. 处理 Sequence Parallel
        if getattr(self.config, "sequence_parallel", False):
            try:
                from megatron.core.tensor_parallel import gather_from_sequence_parallel_region
                aux_hidden = gather_from_sequence_parallel_region(aux_hidden)
            except ImportError:
                logger.warning("[DRAFT-TRAIN-SERIAL] Cannot import gather_from_sequence_parallel_region")

        # 转置 (S, B, H) -> (B, S, H)
        aux_hidden = aux_hidden.transpose(0, 1).contiguous()
        logger.debug(f"[DRAFT-TRAIN-SERIAL] Captured hidden states shape: {aux_hidden.shape}")
        ...
        draft_out = draft(
            input_ids=input_ids,
            hidden_states=aux_hidden,  # 来自当前 step 的 actor forward（冻结）
            loss_mask=eagle_loss_mask,
            attention_mask=None,
            position_ids=position_ids,
            ttt_length=ttt_length,
        ...
        # 5. Teacher logits（确保 detach，防止 actor 梯度回传）
        # 如果 TP>1，需要 gather 生成 full vocab 的 teacher
        if _eagle3_tp_world_size() > 1:
            teacher_logits, _ = self.output_layer(
                hidden_states, weight=output_weight, runtime_gather_output=True
            )
        ...
        # 7. 暂存 draft loss（后续 backward 使用）
        if not hasattr(self, "_eagle3_draft_losses"):
            self._eagle3_draft_losses = []
        self._eagle3_draft_losses.append(loss_out["loss"])
        ...
        logger.warning(f"[DRAFT-TRAIN-SERIAL] Failed: {e}")

        # OOM 兜底
        if "out of memory" in str(e).lower():
            logger.warning("[DRAFT-TRAIN-SERIAL] OOM detected, calling empty_cache()")
            import torch
            if hasattr(torch, "npu") and torch.npu.is_available():
                torch.npu.empty_cache()
            elif hasattr(torch, "cuda") and torch.cuda.is_available():
