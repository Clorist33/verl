...
class PPOTrainer(ABC):
    ...
    def _step_once_parallel(self, metrics: dict, timing_raw: dict, sample_batch_size: int) -> KVBatchMeta:
        ...
        【原有逻辑封装】这是原有的 _step_once 方法内容的完整复制。
        所有逻辑完全不变，只是从原来的 _step_once 移动到这个方法中。
        """
    ...
    def _step_once_serial(self, metrics: dict, timing_raw: dict, sample_batch_size: int) -> KVBatchMeta:
        ...
        if train_actor:
            # === Actor 训练步：禁用 draft ===
            batch.extra_info['enable_draft_training'] = False
            batch.extra_info['train_draft_only'] = False
            ...
            batch.extra_info['enable_draft_training'] = True
            batch.extra_info['train_draft_only'] = True
...
# ========================================
# Serial Training Support (EAGLE3)
# ========================================


class SerialTrainingScheduler:
    """Online 串行训练调度器（EAGLE3 Actor-Draft 串行训练）

    用于判断当前 step 应该训练 actor 还是 draft。

    Example:
        scheduler = SerialTrainingScheduler(actor_steps_per_draft=5)

        if scheduler.should_train_actor(global_step):
            # Actor 训练逻辑
        elif scheduler.should_train_draft(global_step):
            # Draft 训练逻辑
    """
    ...
    def __init__(self, actor_steps_per_draft: int):
        """
        Args:
            actor_steps_per_draft: 每训练多少步 actor 后训练一次 draft（k 值）
                例如 k=5 时，step 1-5 训练 actor，step 6 训练 draft
        """
    ...
    def should_train_actor(self, global_step: int) -> bool:
        ...
        Args:
            global_step: 当前的全局训练步数

        Returns:
            True 表示当前步应该训练 actor，False 表示不应该

        Example:
            当 k=5 时：
            - Step 1-5: True  (训练 actor)
            - Step 6: False   (不训练 actor)
            - Step 7-11: True
            - Step 12: False
        """
    ...
    def should_train_draft(self, global_step: int) -> bool:
        ...
        Returns:
            True 表示当前步应该训练 draft，False 表示不应该

        Example:
            当 k=5 时：
            - Step 1-5: False  (不训练 draft)
            - Step 6: True     (训练 draft)
            - Step 7-11: False
            - Step 12: True
        """
        return (global_step % self.cycle_length) == 0
