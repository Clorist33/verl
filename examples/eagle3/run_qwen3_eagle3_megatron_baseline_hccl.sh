...
export HCCL_IF_BASE_PORT=23100

export RAY_EXPERIMENTAL_NOSET_ASCEND_RT_VISIBLE_DEVICES=1
export TASK_QUEUE_ENABLE=1
export CUDA_DEVICE_MAX_CONNECTIONS=1
# Official MoE (run_qwen3_30b_a3b_megatron.sh) sets this; parity + perf.
export DISABLE_L2_CACHE=1
# HcclAllgather on the etp_group during Megatron->vLLM weight export hit
# "no notify resource" (rtsNotifyCreate 207009) on this MoE+EAGLE3 run. AIV
# dispatch moves comm expansion off the host, easing per-device notify pressure.
...
POLICY_PATH=${POLICY_PATH:-/home/weight/Qwen3-30B-A3B}
DRAFT_PATH=${DRAFT_PATH:-/home/weight/Qwen3-30B-A3B-EAGLE3}

# ===== user-adjustable: hardware (Qwen3-30B-A3B MoE: TP4*EP4 on 16 cards) =====
# MoE is sharded by tensor-parallel (TP) x expert-parallel (EP). Mirrors the
# official run_qwen3_30b_a3b_megatron.sh: 16 cards, TP4/DP4/EP4/ETP4/EDP1.
# Megatron constraint: ETP*EP*EDP == TP*CP*DP -> 4*4*1 == 4*1*4 = 16 (valid).
# DP=4 lets the distributed optimizer shard fp32 state 4-way (the prior 32B-dense
# OOM was DP=1 with the whole fp32 grad buffer on one card -- DP4 avoids that).
# EAGLE3 gate only blocks PP>1 (engine_support.py:143); TP/EP are unrestricted.
NNODES=${NNODES:-1}
...
EAGLE3_ENABLE_ROLLOUT=${EAGLE3_ENABLE_ROLLOUT:-False}    # rollout-side speculative decoding ON by default
...
# ===== parallelism for Qwen3-30B-A3B MoE (mirror official run_qwen3_30b_a3b_megatron.sh) =====
# MoE needs expert parallelism (EP). PP=1 still required by the EAGLE3 gate.
# 16 cards: TP=4 * EP=4, ETP=4 (expert-tensor-parallel), DP=4 implied. 128 experts.
# Env-overridable for the TP escalation sweep. Megatron constraint:
#   ETP*EP*EDP == TP*CP*DP  (16 cards). Pair ETP=TP, keep EP=4:
#     TP=1,ETP=1,EP=4 -> DP=16,EDP=4    TP=2,ETP=2,EP=4 -> DP=8,EDP=2
#     TP=4,ETP=4,EP=4 -> DP=4,EDP=1
train_tp=${TRAIN_TP:-4}
train_pp=1
train_cp=1
train_ep=${TRAIN_EP:-8}
...
# Support both lowercase experiment_name (preferred) and uppercase EXPERIMENT_NAME
experiment_name=${experiment_name:-${EXPERIMENT_NAME:-qwen3_eagle3_megatron}}
...
MODEL=(
    ...
    actor_rollout_ref.model.eagle3.draft_optim_offload=${DRAFT_OPTIM_OFFLOAD:-True}
    actor_rollout_ref.model.eagle3.draft_forward_checkpoint=${DRAFT_FWD_CKPT:-False}
...
ACTOR=(
    ...
    # EAGLE3 hard gate: SP + CP MUST be off (hidden capture needs full sequences).
    # sequence_parallel defaults to True at TP>1, so force it False here or
    # engine_support._assert_no_sp_cp raises at setup.
