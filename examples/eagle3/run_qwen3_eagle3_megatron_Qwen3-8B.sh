...
# ===== EAGLE3 串行训练控制 (显存优化) =====
# 串行训练模式可以降低约 30% 的显存使用，适用于显存紧张的场景
# 原理：Actor 和 Draft 交替训练，避免同时占用计算资源
#
# enable_serial_training:
...
#     周期 6 步，平衡显存优化和训练效率
#   - k=3: 更频繁的 draft 更新，适合 draft 模型较小的情况
#   - k=7: 更少的 draft 更新，最大化显存优化，适合显存极度紧张
