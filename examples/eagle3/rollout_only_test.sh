#!/usr/bin/env bash
# EAGLE3 Rollout-Only 测试脚本 - 只跑 vLLM 推理，不训练
...
EAGLE3_ENABLE_ROLLOUT=${EAGLE3_ENABLE_ROLLOUT:-True}  # 启用 rollout 投机推理
NUM_SPEC_TOKENS=${NUM_SPEC_TOKENS:-3}

# ===== 关键修复：Draft TP 对齐 =====
DRAFT_TP=${DRAFT_TP:-4}  # 修复：从 2 改为 4，与 policy TP 一致
...
DATA=(
    ...
    data.val_files="['$TRAIN_FILE']"  # rollout-only 不需要 val
...
ACTOR=(
    ...
    actor_rollout_ref.actor.optim.lr=0  # rollout-only 模式，lr 设为 0
...
TRAINER=(
    ...
    trainer.save_freq=-1  # rollout-only 不保存 checkpoint
    trainer.test_freq=-1  # rollout-only 不做验证
)
...
echo "  - rollout/acceptance_rate (should be 40-70%)"
echo "  - rollout/mean_acceptance_length (should be 2.5-3.5)"
echo "  - timing_s/gen (generation time)"
echo "  - perf/throughput (tokens/s)"
echo "=========================================="
